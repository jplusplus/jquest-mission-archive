...
MissionQuiz.prototype.data = function(data, callback) {
  ...
  &&  "quiz-solution" in data ) {
    ...
    if( data["quiz-solution"] ) {    
      // Is the answer correct ?
      // Several solution is possible, we have to evalute each of them. 
      data["quiz-solution"].split(",").forEach(function(solution) {
    ...
    self.points +=~~ ( isCorrect ? -1 * Math.pow(data.duration, 3) * 10/125 + 10  : 0 );
    ...
    if(self.questionsLeft <= 0) {
      ...
      self.close(function() {        
        ...
        callback(null, { solution: solutions, isCorrect: isCorrect, isComplete: true});    
      });

    } else {      

      // In any case, send the answer as a JSON document
      callback(null, { solution: solutions, isCorrect: isCorrect, isComplete: false}); 
    }    
