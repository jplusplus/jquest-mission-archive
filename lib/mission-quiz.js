...
function MissionQuiz(models, userId, chapterId, callback) {
  ...
  var defaultValues = {
    ...
    questionsLeft : 0,
    /**
     * Array of questions ID already user. Reset when the mission is open.
     * @type {Array}
     */
    previousQuestions : [],
    /**
...
MissionQuiz.prototype.getRandomQuestion = function() {
  ...
  if(self.previousQuestions.length >= self.questions) {
    // Reset the question saved
    self.previousQuestions = [];
  }

  do {
    
    // Take a random question key
    var key = Math.floor( Math.random() * self.questions.length );

  // Until the key is never use before
  } while( self.previousQuestions.indexOf(key) > -1 );
