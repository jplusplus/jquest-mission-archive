...
function Mission(models, userId, chapterId, next) {        
  ...
  defaultValues = {
    type               : "classic",        // Mission type
    pointsAwarded      : 100,              // Points due when the mission is complete
    duration           : -1,               // Durations in milliseconds of the mission, -1 to disabled  
    templateDirname    : __dirname,        // Path to the template directory
    templateFilename   : "index.jade",     // Name of the template file to load
    templateOptions    : { debug: false},  // Options to use when compiling with Jade
    locals             : {}                // Locals to append to each template parsing 
  };
  ...
  // Final callback
    ...
    // Finaly call the prepare function
    self.prepare.call(self, next);
  });
}
