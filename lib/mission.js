 ...
 */
function Mission(api, user, mission, next) {        
...
Mission.prototype = {
      ...
      self.api.user_progression({
        ...
        if(!err && userProgression.objects.length > 0) {
          ...
          for(var key in self) {
            ...
            if(typeof up[key] !== "undefined" && ! key in exclude) {
  ...
  loadTemplate: function() {
    ...
    // We read the file
    self.templateTxt = fs.readFileSync(self.getTemplatePath(), "utf8");    
  ...
  compileTemplate: function(locals) { 
    ...
    if( ! this.loadTemplate() ) throw new Error("Failed to read the mission's template file.");
  ...
  play: function(next) {      
    // Callback function
    if(typeof next === "function") next.call(this, null, {});
  },
